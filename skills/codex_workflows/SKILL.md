...
Bootstrap selects a tracker (Linear or Azure DevOps Boards), an SCM (GitHub or Azure Repos), provider mappings, and a branch template with `{key}`. Hooks enforce the selected branch convention, one mapped work item per worktree, `in_progress` state before writes, artifact prerequisites, and completion evidence. Provider failures fail closed.
