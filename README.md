...
Provider-neutral workflow orchestration for agent hosts. The plugin keeps the orchestrator focused on discovering skills, invoking them, validating contracts, retrying transient work, and reflecting on artifacts. It does not proxy provider calls.
...
The interactive wizard offers Linear or Azure DevOps Boards, GitHub or Azure Repos, provider scope, and branch presets plus an `other` option. A custom branch template must contain `{key}`. Configuration is written to `.codex-workflows/integrations.json`; reinstallation preserves it.
...
6. Feature/story workflows use the same generic work-item and SCM contracts; vendor-specific details stay inside adapters.
