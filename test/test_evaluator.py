...
class TestEvaluator(unittest.TestCase):
    ...
    def test_valid_output(self):
        self.assertEqual(evaluate_output({"ticket_id": "ENG-1", "transition_required": "in_progress", "mode": "instructions"}, self.manifest), [])
    ...
    def test_collect_critiques_preserves_actor_feedback(self):
        output = {"ticket_id": "ENG-1", "transition_required": "in_progress", "mode": "instructions", "critiques": "needs verification"}
        self.assertEqual(collect_critiques(output, self.manifest), ["needs verification"])
